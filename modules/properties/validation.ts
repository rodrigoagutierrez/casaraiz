...
export const createPropertySchema = z.object({
  ...
  maxHuespedes: z.number().int().min(1).max(16).default(2),
  disponibleDesde: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
  disponibleHasta: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
